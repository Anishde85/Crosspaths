...
io.on('connection', socket => {
  ...
  socket.on('participant-name', (userId,user,roomId) => {
    participants[userId] = user
  })

  socket.on('participant-name-fetch',(userId,roomId) => {
    socket.to(roomId).broadcast.emit('participant-name-send', participants[userId])
  })
})
