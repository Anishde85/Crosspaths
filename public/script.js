...
const chat = document.getElementById('element-7')
const record = document.getElementById('record')

const play = document.getElementsByClassName('games-button')[0];
...
navigator.mediaDevices.getUserMedia({
  ...
  myPeer.on('call', call => {
    ...
    video.controls = true;
    socket.emit('participant-name-fetch', (myId,ROOM_ID) => {
      socket.on('participant-name-send',userName => {
        video.title = 'Ahel';
      })
    })
  ...
  socket.on('user-connected', (userId,userName) => {
    setTimeout(connectToNewUser,1000,userId, stream, userName)
  })
...
myPeer.on('open', id => {
  ...
  myId = id;
  socket.emit('participant-name',myId,user,ROOM_ID)
})


//ADD EXISTING USERS
//TO THE SCREEN OF NEW USER
...
chat.addEventListener('click',function(e){
  numClicksChat++;
  if(numClicksChat % 2 === 1){
    document.getElementById('chat-grid').setAttribute("style","width: 0%")
    document.getElementById('chat-grid').setAttribute("style","display: none")
    document.getElementById('video-grid').setAttribute("style","width: 100%")
  }else{
    document.getElementById('chat-grid').setAttribute("style","width: 30%")
    document.getElementById('chat-grid').setAttribute("style","display: flex")
    document.getElementById('video-grid').setAttribute("style","width: 70%")
  }
})



//Triggers the opening of the panel containing link to the games
...
function closeOpen() {
  ...
  if(games.style.visibility == 'hidden')games.style.visibility = 'visible'
  else games.style.visibility = 'hidden'
}
